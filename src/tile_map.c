...
void tile_map_reassign(Stage* s, int width, int height)
{
    ...
    tmp = realloc(s->tile_map.tiles, sizeof(int) * s->tile_map.width * s->tile_map.height);

    if (tmp != NULL)
    {
        s->tile_map.tiles = tmp;
    }

    tile_map_assign(&s->tile_map, &s->tile_set);
}
...
void tile_map_draw(Game* game, Tile_Map* tile_map, Tile_Set* tile_set)
{
    ...
    if (end_x > tile_map->width) end_x = tile_map->width;
    if (end_y > tile_map->height) end_y = tile_map->height;
